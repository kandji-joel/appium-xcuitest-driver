...
class IOSCrashLog {
  constructor(opts = {}) {
    this.udid = opts.udid;
    this.pymobiledevice3Client = this.udid ? new Pymobiledevice3(this.udid) : null;
  ...
  async _gatherFromRealDevice() {
    if (await this.pymobiledevice3Client?.assertExists(false)) {
      return (await /** @type {Pymobiledevice3} */ (this.pymobiledevice3Client).listCrashes()).map(
      ...
      return (
        await B.map(paths, async (fullPath) => {
          if (_.includes(fullPath, REAL_DEVICE_MAGIC)) {
            ...
            try {
              // @ts-expect-error If pymobiledevice3Client is not defined, then the exception will be caught below
