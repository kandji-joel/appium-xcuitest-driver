...
import Pymobiledevice3 from '../pymobiledevice3-client';
...
export default {
   ...
   *
   * This only works _if and only if_ `pymobiledevice3` is installed on the same machine Appium is running on.
   *
   ...
   * forwards the original pymobiledevice3 output)
   * @throws {Error} If attempting to remove certificates for a simulated device or if `pymobiledevice3` is not installed
   ...
   * @throws {Error} If attempting to list certificates for a simulated device or if `pymobiledevice3` is not installed
   * @this {XCUITestDriver}
   */
